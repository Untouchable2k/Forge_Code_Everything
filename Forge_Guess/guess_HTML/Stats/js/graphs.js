...
function generateHashrateAndBlocktimeGraph(eth, target_cv_obj, era_cv_obj, tokens_minted_cv_obj) {
  ...
  function getHashrateDataFromDifficultyAndErasPerBlockData(difficulty_data, eras_per_block_data) {
    var expected_eras_per_block = 1/170; //76.5/* should be 40 times slower than ethereum (with 15-second eth blocks) */
  ...
  var expected_eras_per_block = 1/220; /* should be 40 times slower than ethereum (with 15-second eth blocks) */
    var difficulty_data_index = 0;
    var difficulty_change_block_num = 0;
    var chart_data = []
    for (var step = 0; step < eras_per_block_data.length; step++) {
...
if(eras_per_block_data[i].x > 24970892){
  ...
  var hr_diff_chart = new Chart.Scatter(document.getElementById('chart-hashrate-difficulty').getContext('2d'), {
    ...
    data: {
        datasets: [{
            ...
            label: "ForgeToken.org Pool Hashrate",
            showLine: true,
      ...
      tooltips: {
        callbacks: {
          label: function(tooltipItem, data) {
            ...
            }else if (data.datasets[tooltipItem.datasetIndex].label == "Forge Hashrate") {
              label +=toReadableHashrate(tooltipItem.yLabel);
            }else if (data.datasets[tooltipItem.datasetIndex].label == "ForgeToken.org Pool Hashrate") {
              label +=toReadableHashrate(tooltipItem.yLabel);
  ...
  /* set y-values to ideal block time */
  datasetCopy2[0].y = _IDEAL_BLOCK_TIME_SECONDS / 60 * 3;
  datasetCopy2[1].y = _IDEAL_BLOCK_TIME_SECONDS / 60 * 3;
  //console.log('datasetCopy', datasetCopy);
  log('showing graph 2');
  /* block time chart */
  var rewardtime_chart = new Chart.Scatter(document.getElementById('chart-rewardtime').getContext('2d'), {
    ...
    data: {
        datasets: [{
          ...
          label: 'Perfect IDEAL Reward Time',
          showLine: true,
          fill: false,
          backgroundColor: 'rgb(226, 15, 162)',
          borderColor: 'rgb(226, 15, 162)',
          borderDash: [5, 15],
          pointRadius: 0,
          data: datasetCopy2,
          yAxisID: 'first-y-axis',
...
async function show_progress(value){
  ...
  el('#difficultystats').innerHTML = '<div class="">Loading2 info from the blockchain... <span style="font-weight:600;">' + value + '</span></div>';
}
...
log("last mining_target_values ",mining_target_values)
