...
/* intrinsic values */
/* intrinsic values */
const _SECONDS_PER_ETH_BLOCK = 2.162;
...
const _CONTRACT_ADDRESS = "0xF44fB43066F7ECC91058E3A614Fb8A15A2735276";
...
var eth = new Eth(new Eth.HttpProvider("https://polygon-mainnet.g.alchemy.com/v2/175MhB02loz6iTxgsAUQDmqnGTo6jHQf"));
...
// }
var _BLOCK_EXPLORER_ADDRESS_URL = 'https://polygonscan.com/address/';
var _BLOCK_EXPLORER_TX_URL = 'https://polygonscan.com/tx/';
var _BLOCK_EXPLORER_BLOCK_URL = 'https://polygonscan.com/block/';
...
stats = [
 ...
 ['Last Difficulty Start Block',   token.latestDifficultyPeriodStarted,  "",                 1,          null     ], /* mining difficulty */
  ...
  ['',                              null,                                 "",                 1,          null     ], /* */
['Tokens distributed via Mining',                 token.tokensMinted,                   _CONTRACT_SYMBOL,   0.000000000000000001, null     ], /* supply */
 ['Tokens distributed via Staking',                 token.tokensMinted,                   _CONTRACT_SYMBOL,   0.0000000000000000005, null     ], /* supply */
 ['Tokens distributed via Auctions',                 token2.currentDay,                   _CONTRACT_SYMBOL,   8192, null     ], /* supply */
  ['',                              null,                                 "",                 1,          null     ], /* */
  
  ['Total Forge Circulating Supply',     token.tokensMinted,                                 "Forge",          0.0000000000000000015,          null     ], /* mining difficulty */
 ['Inflation Percentage Per Year', token.latestDifficultyPeriodStarted,  "",                 1,          null     ], /* supply */
 
['Max Supply for Current Era',    token.maxSupplyForEra,                _CONTRACT_SYMBOL,   0.000000000000000001976, null     ], /* mining */ 
  
['Total Supply',                  token.totalSupply,                    _CONTRACT_SYMBOL,   0.000000000000000001, null     ], /* supply */
    
['',                              null,                                 "",                 1,          null     ], /* */
  ['Current Auction',                 token2.currentDay,                   "",   1, null     ], /* supply */
 ['Time Remaining in Auction',  token2.nextDayTime,                   "",   1, null     ], /* supply */
 ['Auction Length',  token2.secondsPerDay,                   "Days",   0.00001157407, null     ], /* supply */
 ['Total 0xBitcoin spent at Auctions',  token2.totalBurnt,                   "0xBTC",   0.00000001, null     ], /* supply */
  ['',                              null,                                 "",                 1,          null     ], /* */
 ['Liquidity',                token4.getReserves,                      "",                 0.000000000000000001,          null     ], /* mining */
...
['Current Mining Reward per 48 minute Solve',         token.getMiningReward,                _CONTRACT_SYMBOL,   0.000000000000000001, null     ], /* mining */
...
['Token Holders',                 null,                                 "holders",          1,          null     ], /* usage */
 ...
 ['u2',     token2.secondsPerDay,                                 "",              1,          null     ], /* usage */
 ['u4',   token4.getReserves,  "",                 1,          null     ], /* mining difficulty */
  ['u5',     null,                                 "txs",              1,          null     ], /* usage */
 ['u6',                eth.blockNumber,                      "",                 1,          null     ], /* mining */
   ['u7',   token3.totalSupply,  "",                 1,          null     ], /* mining difficulty */
   ['u8',   token3.rewardRate,  "",                 1,          null     ], /* mining difficulty */
   ['u9',   token3.rewardRate2,  "",                 1,          null     ], /* mining difficulty */
...
test5 = rewards_blocks_remaining_in_era.toLocaleString().indexOf(".")
if(test5 != -1){
	rewards_blocks_remaining_in_era2 = rewards_blocks_remaining_in_era.toLocaleString().slice(0,test5)
	if(supply_remaining_in_era2.length < 3){
	...
	rewards_blocks_remaining_in_era = rewards_blocks_remaining_in_era.toLocaleString()
}

console.log("test5, ",supply_remaining_in_era)
if(test5 != -1){
	
  el_safe('#MinedSupplyRemaininginEra').innerHTML = "<b>" + supply_remaining_in_era2 + "</b> " + _CONTRACT_SYMBOL + " <span style='font-size:0.8em;'>(" +rewards_blocks_remaining_in_era2 + " blocks)</span>";

}else{
  el_safe('#MinedSupplyRemaininginEra').innerHTML = "<b>" + supply_remaining_in_era.toLocaleString() + "</b> " + _CONTRACT_SYMBOL + " <span style='font-size:0.8em;'>(" +rewards_blocks_remaining_in_era2.toFixed(0) + " blocks)</span>";
}
...
console.log("First :", rewards_blocks_remaining_in_era);
console.log("First _IDEAL_BLOCK_TIME_SECONDS:", _IDEAL_BLOCK_TIME_SECONDS);

  liqForge = getValueFromStats('Liquidity', stats)	


  el_safe('#Liquidity').innerHTML = "<b>" + (liqForge).toFixed(0) + " </b> Forge  ";



  liq0xx = getValueFromStats('u4', stats) /100000000
testzzf  = liq0xx.toFixed(0)
fiz = String(testzzf).toLocaleString()
console.log("fiz :", fiz);
console.log("fiz testzzf:", fiz);

  el_safe('#Liquidity').innerHTML += "& <b>" + fiz + " </b> 0xBitcoin";


  perstaked = getValueFromStats('Percent of Liquidity that is Staked', stats)

  perstaked2 = getValueFromStats('u7', stats) *100 / perstaked


  rewardRates2 = getValueFromStats('u9', stats)



  el_safe('#PercentofLiquiditythatisStaked').innerHTML = "<b>" + perstaked2.toFixed(2) + " </b> %";


rewardRates = getValueFromStats('u8', stats)

yearly = rewardRates /(10**34) * (60*60*24*365)

yearly2 = rewardRates2 /(10**24) * (60*60*24*365)

console.log("yearly", yearly);

console.log("yearlyrewardRates", rewardRates)
apy = yearly / (perstaked2 * liqForge*2/100)  * 100
apyz2 = yearly2 / (perstaked2 * liq0xx*2/100)  * 100

console.log("yearlyapy", apy);
console.log("yearlyapy2", apyz2);
apy = apy + apyz2
  el_safe('#StakingAPY').innerHTML = "<b>" + apy.toFixed(2) + " </b> %";

//   ['u9',   token3.rewardRate2,  "",                 1,          null     ], /* mining difficulty */
// liq0xx for 0xBTC liquidity
  /* time until next epoch ('halvening') */
console.log("yearlyapy22", rewards_blocks_remaining_in_era);


  freadsf = getValueFromStats('Tokens distributed via Mining', stats)


test5 = freadsf.toLocaleString().indexOf(".")
if(test5 != -1){
	freadsf2 = freadsf.toLocaleString().slice(0,test5)
	if(freadsf2.length < 3){
	...
	freadsf2 = freadsf.toLocaleString()
}

  el_safe('#TokensdistributedviaMining').innerHTML = "<b>" + freadsf2 + " </b> Forge";
  freadsffff = getValueFromStats('Auction Length', stats)

  el_safe('#AuctionLength').innerHTML = "<b>" + freadsffff.toFixed(1) + " </b> days";

  freadsf2 = getValueFromStats('Current Mining Reward per 48 minute Solve', stats)

  el_safe('#CurrentMiningRewardper48minuteSolve').innerHTML = "<b>" + freadsf2.toFixed(0) + " </b> Forge";


  freadsf = getValueFromStats('Tokens distributed via Staking', stats)
...
if(test5 != -1){
  ...
  el_safe('#TokensdistributedviaStaking').innerHTML = "<b>" + freadsf2 + " </b> Forge";
  auction_tot2 = getValueFromStats('Total 0xBitcoin spent at Auctions', stats)
  
  el_safe('#Total0xBitcoinspentatAuctions').innerHTML = "<b>" + auction_tot2.toFixed(0) + " </b> 0xBTC";
  ...
  circl = getValueFromStats('Total Forge Circulating Supply', stats) + getValueFromStats('Current Auction', stats)*8192-8192
...
if(test51 != -1){
  ...
  el_safe('#TotalForgeCirculatingSupply').innerHTML = "<b>" + circl2 + " </b> Forge";



  auction_totf1 = getValueFromStats('Tokens distributed via Auctions', stats) - 8192


  el_safe('#TokensdistributedviaAuctions').innerHTML = "<b>" + auction_totf1.toLocaleString() + " </b> Forge";
...
const start = Date.now();
 timeremain = getValueFromStats('Time Remaining in Auction', stats)- start/1000 
 tt2 = freadsffff*24*3600 - timeremain
 if(timeremain > 3600*24){
	 el_safe('#TimeRemaininginAuction').innerHTML = "<b>" + '<progress value="'+tt2+'" max="'+freadsffff*24*3600+'"></progress>' +(timeremain/(3600*24)).toFixed(1) + " </b> days";
}else if(timeremain > 3600){
el_safe('#TimeRemaininginAuction').innerHTML = "<b>" +'<progress value="'+tt2+'" max="'+freadsffff*24*3600+'"></progress>'+(timeremain/(3600)).toFixed(1) + " </b> hours";
}else if(timeremain > 60){
el_safe('#TimeRemaininginAuction').innerHTML = "<b>" + '<progress value="'+tt2+'" max="'+freadsffff*24*3600+'"></progress>'+(timeremain/(60)).toFixed(1) + " </b> minutes";
}else {
el_safe('#TimeRemaininginAuction').innerHTML = "<b>" + '<progress value="'+tt2+'" max="'+freadsffff*24*3600+'"></progress>'+(timeremain).toFixed(1) + " </b> seconds";
}
...
//GET THE FUCKING RATIO BABY
  el_safe('#CurrentRewardEra').innerHTML += " <span style='font-size:0.8em;'>(next era: ~" + secondsToReadableTime(rewards_blocks_remaining_in_era * _IDEAL_BLOCK_TIME_SECONDS * 1 / rad * 1.1) +")</div>";

log("mine_length", mine_Length2)
 /*Get Inflation % per year */
  auction_Length = getValueFromStats('u2', stats)
  ...
  seconds_Total = blocks_Total * 2.5
  current_Reward = getValueFromStats('Current Mining Reward per 48 minute Solve', stats)
log("rew",seconds_Total) 
  epoch_From = getValueFromStats('Epoch Count', stats) % 256
  total_Minted = getValueFromStats('Total Forge Circulating Supply', stats)
  year_Time = 60*60*24*365
log("xlength2seconds_per_reward",seconds_per_reward)
  x =  seconds_per_reward * 888 / (48*60);
log("xlength2",x)

log("xlength3",x)
log("xlength3z",(61001200 * (x ^ 2 ))/(888 ^ 2))
 finalx = (61001200 * (x ^ 2 ))/(888 ** 2) + (40861500 * x)/(888);
log("xlength", finalx/100000000)
...
current_Reward = current_Reward * finalx/100000000
  ...
  inflation = ((8192 / auction_Length) + ((current_Reward * epoch_From) / seconds_Total )) * year_Time
log("auction_Length", 8192/auction_Length*year_Time) 
log("auction_Length2", (current_Reward * epoch_From) / seconds_Total*year_Time) 
log("auction_Length2inflation",inflation) 
log("auction_Length2inflation2",total_Minted) 
...
function updateThirdPartyAPIs() {
      ...
      el('#TokenHolders').innerHTML = "<b> 22</b> holders";
      el('#TokenTransfers').innerHTML = "<b> 1000</b> transfers";


      el('#TotalContractOperations').innerHTML = "<b> 20000</b> txs";

  $.getJSON('https://raw.githubusercontent.com/Untouchable2k/Forge-Main/main/holdersandtran.html',
    function(data) {
...
if(stat[0]!='u2' && stat[0] != 'u33' && stat[0] != 'u4' && stat[0] != 'u5' && stat[0] != 'u6' && stat[0] != 'u7' && stat[0] != 'Last Eth Block' && stat[0] != 'u8' && stat[0] != 'u9'){
