  ...
  async init(web3,tokenContract,mintHelperContract, poolConfig,accountConfig,redisInterface,tokenInterface)
  {
    ...
    this.mergeMintTokens = this.poolConfig.mergeMintTokens;
      ...
      var self=this;
        ...
        setTimeout(function(){self.broadcastQueuedTransactions()},500)
  //      setTimeout(function(){self.resendUnbroadcastedPayments()},0)
        setTimeout(function(){self.updateBroadcastedTransactionStatus()},30000)
  },
...
//types are 'transfer','solution'
  async addTransactionToQueue(txType,txData)
  {
		...
		var gaz5 = await this.web3.eth.getGasPrice()
          var challengeNumber2 = await this.redisInterface.loadRedisData('lastChallengesubmit2' )
	var seconds = new Date().getTime() / 1000;
	console.log("CHL FIRST: ", challengeNumber2);
	console.log("CHL gaz5: ", gaz5);
	  if(gaz5 < 262900000000 && challengeNumber2 + 90 <= seconds){
	  await this.redisInterface.storeRedisData('lastChallengesubmit2', seconds);
	
	
	console.log("CHL 2: ", txData.challenge_number);
	//if(txData.challenge_number != GChallenge)
	//{
	//console.log("This: ", txData.challenge_number);
	//console.log(" Vs THIS: ", GChallenge);

	//GChallenge = txData.challenge_number
	//GChallenge2 = txData.challenge_number
	//add to redis
	    var receiptData = {
	      queued:true,
	      pending:false,
	    ...
	    var blockNum  = await this.getEthBlockNumber();
	  	console.log("PLEASE", txData.challenge_number);
	    var packetData = {
	      block: blockNum,
	      txType: txType,
	      txData: txData,
	    ...
	    //var packetDataJSON = await this.redisInterface.getResultsOfKeyInRedis2('active_transactions');
		//console.log("hash: ", packetDataJSON);
   ...
   async updateBroadcastedTransactionStatus()
   {
    ...
    if (!cluster.isMaster) { return; }
     ...
     }catch(e)
     {
      ...
      setTimeout(function(){self.updateBroadcastedTransactionStatus()},200)
   },
...
//        console.log('transactionPacketsData',transactionPacketsData)
   ...
   async getWorkingMergeMintAccounts(solution_number, challenge_digest, max_gas_cost, addressFrom, addressTo){
    ...
    console.log("not merge mint");
     ...
     var epochCount = await this.tokenInterface.getEpochCount();
	console.log("EPOOCH COUNT: ", epochCount);
	if(epochCount % 2 == 1){

		const cars = ["0x578360AdF0BbB2F10ec9cEC7EF89Ef495511ED5f"];
		...
		var txData = this.web3.eth.abi.encodeFunctionCall({
          ...
          }, [solution_number, challenge_digest, cars, "0xdFD18d5374dF275A8EDE107f981f78b1E60f63E4"]);
 ...
 var txData = this.web3.eth.abi.encodeFunctionCall({
	...
	//change this back aftger running for awhile just for launch
      var gas = await this.web3.eth.getGasPrice()*1.1
	console.log("gas" , gas)
      gas = Math.min(gas,243990000000)
     var gas2 = 3990000000
      gas = gas.toFixed(0)
    ...
    maxFeePerGas: web3Utils.toHex( web3Utils.toWei( gas.toString() , 'wei' ) ),
    maxPriorityFeePerGas: web3Utils.toHex( web3Utils.toWei( gas.toString() , 'wei' ) ),
        value: 0,
        to: addressTo,
        data: txData,
    ...
    console.log(txOptions);
      const tx = FeeMarketEIP1559Transaction.fromTxData( txOptions, { chain: 'polygon', hardfork: 'london' }  );
    ...
    console.log(await web3.eth.net.getId());
      const serializedTx2 = signedTransaction.serialize().toString('hex')
        ...
        try
        {
          var result =  web3.eth.sendSignedTransaction('0x' + serializedTx2, callback)
